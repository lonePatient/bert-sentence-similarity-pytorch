...
configs = {
    'arch':'bert',
    'raw_data_path1': path.sep.join([BASE_DIR,'dataset/raw/atec_nlp_sim_train.csv']),   # 总的数据，一般是将train和test何在一起构建语料库
    'raw_data_path2': path.sep.join([BASE_DIR,'dataset/raw/atec_nlp_sim_train_add.csv']),   # 总的数据，一般是将train和test何在一起构建语料库
    'raw_data_path':path.sep.join([BASE_DIR,'dataset/raw/train_join.csv']),
    ...
    'vocab_path': path.sep.join([BASE_DIR, 'model/pretrain/chinese_L-12_H-768_A-12/vocab.txt']),
    'tf_checkpoint_path': path.sep.join([BASE_DIR, 'model/pretrain/chinese_L-12_H-768_A-12/bert_model.ckpt']),
    'bert_config_file': path.sep.join([BASE_DIR, 'model/pretrain/chinese_L-12_H-768_A-12/bert_config.json']),
    ...
    'max_seq_len': 40 * 2,  #这里，我们训练是一对句子对 word文本平均长度,按照覆盖95%样本的标准，取截断长度:np.percentile(list,95.0)
    ...
    'learning_rate': 3e-5,
    'n_gpus': [1], # GPU个数,如果只写一个数字，则表示gpu标号从0开始，并且默认使用gpu:0作为controller,
