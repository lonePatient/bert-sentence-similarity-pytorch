...
class Trainer(object):
    ...
    def _valid_epoch(self):
        ...
        with torch.no_grad():
            for step, (input_ids, input_mask, segment_ids, label_ids) in enumerate(self.val_data):
                ...
                acc = self.evaluate(output=logits, target=target)

                self.valid_f1.update(output=logits, target=target)
    ...
    def _train_epoch(self):
        ...
        for step, (input_ids, input_mask, segment_ids, label_ids) in enumerate(self.train_data):
            ...
            acc = self.evaluate(output=logits, target=target)
            # 更新训练指标
            self.train_f1.update(output=logits, target=target)
